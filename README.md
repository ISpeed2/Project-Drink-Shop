...
База данных “Project Drink Shop” предназначена для хранения информации о товарах, категориях и брендах.

**Таблицы:**

Products: Содержит информацию о товарах.

ProductId (SERIAL PRIMARY KEY): Уникальный идентификатор товара (автоинкрементное целое число).
...
ImageUrl (VARCHAR(255)): URL изображения товара.

CategoryId (INT REFERENCES Categories(CategoryId)): ID категории товара (внешний ключ, ссылающийся на таблицу Categories).

BrandId (INT REFERENCES Brands(BrandId)): ID бренда товара (внешний ключ, ссылающийся на таблицу Brands).

Categories: Содержит информацию о категориях товаров.

CategoryId (SERIAL PRIMARY KEY): Уникальный идентификатор категории (автоинкрементное целое число).

Name (VARCHAR(255) NOT NULL): Название категории.

Description (TEXT): Описание категории.

Brands: Содержит информацию о брендах товаров.

BrandId (SERIAL PRIMARY KEY): Уникальный идентификатор бренда (автоинкрементное целое число).
...
Products -> Categories: Один-ко-многим (одна категория может содержать много товаров).

Products -> Brands: Один-ко-многим (один бренд может содержать много товаров).
